...
public class PlayerController_2D : MonoBehaviour
{
  ...
  public Transform movePointLocation;
    ...
    if (useMouse)
    {
      ...
      if (Input.GetMouseButtonDown(1))
      {
        ...
        if (Physics.Raycast(intoScreen, out hitInfo, 1000, floorLayer))
        {
          ...
          {
            movePointLocation.position = -hitInfo.point + new Vector3(0, 1, 0);
      ...
      if (moveToMouse)
      {
        transform.LookAt(movePointLocation);

        Vector3 newDirection = movePointLocation.transform.position;
        ...
        if (Vector3.Distance(newDirection, transform.position) <= 1)
        {
          moveToMouse = false;
        }
